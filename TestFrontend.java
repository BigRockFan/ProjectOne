...
public class TestFrontend {
        ...
        public void runTests() {
                ...
                System.out.print("Test frontend initially lists no movie (WARNING: if 'x' does not exit app, test won't exit and run indefinitely!): ");
                if (this.testFrontendInitialOutputNoMovieWithTwoMovies()) {
                    System.out.println("PASSED");
                } else {
                    System.out.println("FAILED");
                }
                System.out.print("Test 'r' load rating  selection screen (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinetely!): ");
                if (this.testFrontendRating()) {
                    System.out.println("PASSED");
                } else {
                    System.out.println("FAILED");
                }
...
ne Perez,,\"Michael Paré, Wilma Elles, Joseph Camilleri, Rebecca Tarabocchia, Jeanine Harrington, Malorie Glavan, Dan
...
\",5.4\n"
         ...
         * This test runs the front end and redirects its output to a string. It then
         * passes in 'x' as a command to exit the app. The test succeeds if the front
         * end does not contain any of the two  movies (the movie titles are not in
         * the string captured from the front end) by default. It fails if any of
         * those two  titles are present in the string or an exception occurs.
         * @return true if the test passed, false if it failed
         */
        public boolean testFrontendInitialOutputNoMovieWithTwoMovies() {
                PrintStream standardOut = System.out;
                InputStream standardIn = System.in;
                try {
                        // set the input stream to our input (with an x to test of the program exists)
...
ion_company,actors,description,avg_vote\n"
                                        + "The Insurrection,The Insurrection,2020,Action,90,USA,English,Rene Perez,Re
ne Pe
rez,,\"Michael Paré, Wilma Elles, Joseph Camilleri, Rebecca Tarabocchia, Jeanine Harrington, Malorie Glavan, Danner B
oyd, 
...
ckgrounds defy their parents and friends to stay together. A musical adaptation of the 1983 film.\",5.4\n"
         ...
         * The test succeeds if the genre selectio screen contains all five genres
         * from the data. It fails if any of them are missing, the front end has not
         * been instantiated (is null), or there is an exception. The first test method
         * was for three movies but in this example it will only use One movie. 
         * @return true if the test passed, false if it failed
         */
        public boolean testFrontendRating() {
                PrintStream standardOut = System.out;
                InputStream standardIn = System.in;
                try {
                        // set the input stream to our input (with an g to test of the program lists genres)
...
ogether by one of our most primal fears, the fear of the unknown.\",3.5\n"
                     ...
                     // set the output back to standard out for running the test
                        ...
                        // add all tests to this method
                        String appOutput = outputStreamCaptor.toString();
                        int newAppOutput = Integer.parseInt(appOutput);
                        if (frontend != null && newAppOutput > 10
                            || newAppOutput<0 ) {
                                // test passes if all genres from the data are displayed on the screen
                                return true;
                        } else {
